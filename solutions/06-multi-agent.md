...
### Q: AI Agent 中 Agents、Teams、Workflows 三者有哪些区别？在什么场景下应该选择单 Agent、Agent Team 或固定 Workflow？

#### 1. 网络整合回答 / Comprehensive Answer

这三个概念最容易混在一起，但核心差别在于“决策权在哪里”以及“执行路径是不是预先确定”。  
`Agent` 是单个具备目标、上下文、工具和循环执行能力的智能体，它会根据环境反馈动态决定下一步做什么；`Team` 是多个 Agent 按角色分工协作，例如 planner、researcher、coder、reviewer，通过协调机制共同完成任务；`Workflow` 则是预先设计好的固定流程，步骤、分支、审批点和终止条件大多由工程系统明确写死，LLM 只是其中的一个节点。  
选择时可以用一个简单判断：如果任务路径稳定、审计要求高、业务规则清楚，优先用 Workflow；如果任务开放、步骤难以预先枚举，但一个智能体足以完成，优先用单 Agent；如果任务天然需要多角色视角、并行探索或专业分工，再考虑 Agent Team。实际生产里最常见的不是纯粹三选一，而是“Workflow 做骨架，Agent 做动态节点，Team 处理少数复杂子任务”的混合形态。  
相关章节 / Related Topics: `03-design-patterns`, `10-real-world-cases`。

#### 2. 结合实际例子 / Practical Example

以“处理一个复杂 PR”为例：

- 固定 Workflow：拉取代码、运行测试、检查 lint、生成报告，这些步骤完全可以写成确定性流水线。
- 单 Agent：当用户说“帮我定位这个回归 bug”，一个代码 Agent 可以自主查日志、读代码、改补丁、跑测试。
- Agent Team：如果改动跨前后端、数据库和文档，可能需要 planner 拆任务，coder 改代码，reviewer 查风险，tester 验证回归。

这也是为什么很多成熟系统不是一上来就多 Agent，而是先用 Workflow 把稳定部分收紧，再把真正不确定的部分交给 Agent。

#### 3. 面试核心回答 / Core Interview Answer

- Agent 强在动态决策，Workflow 强在确定性执行，Team 强在角色分工与并行协作。
- 路径稳定、可审计要求高的任务优先 Workflow。
- 开放式任务优先单 Agent，跨角色复杂任务再上 Team。

一句话总结：三者不是同义词，而是三种不同复杂度和控制粒度的系统组织方式。
...
### Q: 多 Agent / 多异步任务场景下，如何防止上下文污染（Context Pollution）？

#### 1. 网络整合回答 / Comprehensive Answer

上下文污染指的是：某个 Agent、某个异步分支、某轮历史结果把不该共享的信息带进了当前决策，导致模型理解偏移、串任务、重复执行或把旧结论当新事实。  
在多 Agent / 多异步系统里，这个问题比单 Agent 更严重，因为系统里往往同时存在共享状态、局部记忆、摘要、工具返回、异步回调和人工反馈。只要边界没划清，一个分支的中间产物就可能污染另一个分支。  
防污染的核心做法是“上下文分域 + 写入受控 + 聚合有规则”。第一，给每个任务、每个 Agent、每个用户会话建立独立的上下文命名空间，不要所有内容都往同一个共享黑板里堆。第二，区分只读上下文和可写上下文，避免所有 Agent 都能改共享状态。第三，跨 Agent 传递尽量用结构化摘要，而不是整段对话和大段自然语言。第四，对共享状态更新设置 owner 和版本号，防止并发写覆盖。第五，对长任务做 TTL、裁剪和摘要更新，避免旧上下文无限滞留。  
真正成熟的系统一般不会让 Agent 自由“互相聊天到解决为止”，而是通过编排层明确：谁能读什么、谁能写什么、谁负责最终汇总。

#### 2. 结合实际例子 / Practical Example

一个多 Agent 的售后系统可以这样做：

- `planner` 只读用户请求和订单状态，只写任务计划。
- `policy_agent` 只读知识库和计划，不直接改共享工单状态。
- `executor` 才拥有写工单和发起退款的权限。
- 各 Agent 之间传递的是结构化字段，如 `intent`、`risk_level`、`next_action`，而不是把整段思考过程互相转发。

这样即使某个检索分支返回了错误摘要，也不会直接污染写操作分支。

#### 3. 面试核心回答 / Core Interview Answer

- 防上下文污染，先做任务隔离和上下文命名空间隔离。
- 共享状态要有限写权限、带版本和 owner，不能谁都能改。
- 跨 Agent 传递尽量结构化和摘要化，不要原样转发全部上下文。

一句话总结：多 Agent 的上下文治理，重点不是“共享更多信息”，而是“只共享该共享的信息”。
